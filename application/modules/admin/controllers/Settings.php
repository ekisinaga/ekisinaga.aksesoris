...
class Settings extends CI_Controller {
    ...
    {
        $fields = array(
            'store_name', 'store_logo', 'store_phone_number', 'store_email', 'store_tagline', 'store_description',
            'store_address', 'min_shop_to_free_shipping_cost', 'shipping_cost','picture'
        );
            ...
            if ( isset($_FILES['store_logo']) && @$_FILES['store_logo']['error'] == '0')
            {
                if ( $this->upload->do_upload('store_logo'))
                {
                    ...
                    $profile_store_logo = $new_file_name;

                    if ( file_exists('assets/uploads/sites/'. $current_profile_store_logo))
                        unlink('./assets/uploads/sites/'. $current_profile_store_logo);
                }
            }
            else
            {
                $profile_store_logo = $current_profile_store_logo;
            }
